...
{ 
	...
	// 999 represents an Infinite value
...
//using adjacency cost matrix representation
...
{  
    ...
    //(We need to a first vertex, so we set its key value to 0 so that it can be added to MST)
    ...
    {  
        ...
        for (int v = 0; v < V; v++)  
        {
            ...
            {  
                parent[v] = u; 
                //set the parent of c to f
                key[v] = cost[u][v];  
                //Change the cost of from 999 to cost[u][v]
            }        
        }
    }
